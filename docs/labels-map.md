...
ترتیب اولویت در تابع `bot_label($key, $lang, $default)` (در `function.php`):

1. **`botlabels` (DB):** override ویرایش‌شده از پنل تحت‌وب (به‌ازای هر زبان).
2. **`lang/<lang>.php` → `textbot`:** مقدار پیش‌فرض فایل زبان.
3. **`$default` یا خود کلید:** اگر هیچ‌کدام نبود.
