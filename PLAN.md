...
> ## ⚡️ حالت فعال توسعه: `PANEL_MODE=shop` (آنلاین‌شاپ)
> **هر تسک این فایل، مگر صریحاً خلافش گفته شود، برای حالت `shop` طراحی، پیاده و تست می‌شود.**
> VPN فقط میراث محافظت‌شده است (صفر رگرسیون)، نه مسیر توسعه. جزئیات کامل و قواعد الزامی
> در [`PROJECT_VISION.md`](PROJECT_VISION.md) → بخش «⚡️ حالت فعال توسعه».
> اگر تمرکز پروژه عوض شد، این بنر + آن بخش باید در همان PR به‌روز شوند.
...
- [x] استپ ۱۲ — ویرایشگر بصری دکمه‌ها (Visual Button Flow) — جایگزین سیستم دکمهٔ تخت
  ...
  - [x] استپ ۱۲ت — وارد کردن «منوی واقعی ربات» به‌صورت درخت + روشن/خاموش + الگوی نمایشی  ✅ 2026-06-08
    - بازخورد کاربر: «منوی اصلی» نود ساختگی گیج‌کننده بود؛ import فقط دکمه‌های automation
      را می‌آورد نه کل منوی واقعی ربات. خواسته: کل منو درخت شود، بشود روشن/خاموش کرد و فرزند افزود.
    - [x] **برچسب ریشه شفاف شد:** `flow_default_tree()` نود ریشه را با برچسب
      «🏠 شروع ربات (ریشه)» و `node_kind='root'` می‌سازد + پیام توضیحی، تا با منوی واقعی اشتباه نشود.
    - [x] **وارد کردن منوی واقعی (`keyboardmain`):** `flow_main_menu_nodes()` ردیف‌های
      منوی اصلی را می‌خواند، هر کلید نمادین (`text_sell`…) را با `bot_label`/lang به برچسب
      واقعی resolve می‌کند و هرکدام را به‌صورت نود `node_kind='system_menu'` زیر ریشه می‌سازد
      (`flow_main_menu_map` نگاشت کلید→callback→قابلیت toggle).
    - [x] **روشن/خاموش از خود فلو:** کلید روشن/خاموش روی نودهای منو (`FlowNode`/`toggleMenuNode`).
      خاموش‌ها خاکستری می‌شوند؛ `flow_normalise_tree` فهرست `meta.disabled_menu_keys` را از روی
      همان فلگ‌ها می‌سازد و `keyboard.php` هنگام render دکمهٔ خاموش را از منوی واقعی ربات حذف
      می‌کند — بدون دست‌زدن به منطق اصلی. دکمه‌های حیاتی (حساب کاربری/پشتیبانی) قفل (`can_toggle=false`).
    - [x] **افزودن فرزند به نود منو:** مجاز است (کشیدن پیوند از پورت پایین) → نود کاربری معمولی
      ساخته می‌شود؛ ویرایش/حذف برچسب نود منو ممنوع (به منطق ربات وصل است).
    - [x] **نودهای نمایشی (الگو):** `flow_demo_submenu_nodes()` منوی تو‌در‌توی پنل مدیریت
      (وضعیت/مدیریت پنل/کاربران/مالی/گزارش…) را به‌صورت `node_kind='system_demo'` فقط‌خواندنی
      می‌سازد تا کاربر ساختار را ببیند و الگو بگیرد؛ غیرقابل ویرایش/حذف/افزودن‌فرزند.
    - [x] سه `node_kind`: `system_menu` (آبی، toggle+فرزند)، `system_demo` (خاکستری، فقط‌خواندنی)،
      `user` (آزاد کامل). Legend و CSS و راهنمای دو‌مرحله‌ای به‌روزرسانی شد.
    - [x] تست‌ها: `flow_main_menu_nodes` (۵ دکمه، resolve برچسب، خاکستری‌شدن، قفل toggle)،
      `flow_build_from_legacy` (root=۱، menu≥۵، demo≥۲، user=تستس)، حفظ/استخراج
      `disabled_menu_keys` — همگی PASS. `php -l` و `node --check` و توازن براکت‌ها OK.

  - [x] استپ ۱۲ث — رفع منوی دوتایی در runtime + کارکرد فرزندِ دکمهٔ منو در هر دو حالت کیبورد  ✅ 2026-06-08
    - بازخورد کاربر روی ربات live: بعد از import، `/start` علاوه بر «خوش‌آمد» پیامِ راهنمای
      نود ریشه + کل منو را هم به‌صورت **دکمهٔ شیشه‌ای (inline)** نشان می‌داد (منوی دوتایی)؛ و
      فرزند «تستی۱» فقط با نسخهٔ inline دکمهٔ «سرویس‌های من» کار می‌کرد نه با نسخهٔ کشویی (reply).
    - ریشه‌یابی: در `/start`، فلو نود ریشه را با `flowhome` دوباره رندر می‌کرد؛ نود ریشه حالا
      کل منوی واقعی را به‌عنوان فرزند دارد، پس همه به‌صورت inline تکرار می‌شدند و پیام راهنمای
      ریشه نشت می‌کرد.
    - [x] **حذف رندر دوبارهٔ ریشه در `/start`** (index.php): دیگر `flowhome` صدا زده نمی‌شود؛
      فقط state کهنه پاک می‌شود. منوی واقعی همان `$keyboard` بومی است.
    - [x] **فیلتر نودهای آینه‌ای در runtime** (`flow_runtime_keyboard`): فرزندان
      `system_menu`/`system_demo`/`root` و نودهای `disabled` دیگر به‌صورت دکمهٔ inline رندر
      نمی‌شوند (منوی واقعی خودش آن‌ها را دارد). فقط نودهای `user` رندر می‌شوند.
    - [x] **follow-up دکمهٔ منو** (`flow_runtime_menu_followup` + `flow_find_menu_node`): با زدن
      هر دکمهٔ منوی واقعی (چه inline با callback مثل `buy`، چه reply با متن دکمه)، اگر ادمین
      فرزند `user` به آن وصل کرده باشد، یک پیام دنبال‌کننده با همان فرزندان فرستاده می‌شود —
      بدون override رفتار بومی دکمه. در index.php قبل از dispatch اصلی صدا زده می‌شود (non-blocking،
      با رد کردن `flow*` و `/start`). این کار فرزندِ دکمهٔ منو را در **هر دو حالت** کیبورد فعال می‌کند.
    - [x] تست‌ها: `flow_find_menu_node` (by callback/label، رد دمو/ناشناخته)، فیلتر
      `flow_runtime_keyboard` (ریشه صفر دکمه، نود منو فقط فرزند user، رد disabled) — همگی PASS.
      `php -l` همهٔ فایل‌ها OK.

  - [x] استپ ۱۲ج — انتخاب نحوهٔ نمایش دکمه‌های فرزند هر نود: شیشه‌ای / کشویی / هر دو  ✅ 2026-06-08
    - درخواست کاربر: گاهی برای یک نود فقط پنل کشویی (reply) و گاهی فقط پنل شیشه‌ای (inline)
      لازم است، گاهی هم هر دو. باید مثل تیک‌های «بازگشت/منوی اصلی خودکار» باشد ولی **همیشه
      حداقل یکی فعال بماند** (نمی‌شود هیچ‌کدام انتخاب نشود).
    - [x] **مدل داده** (`flow.php` → `flow_normalise_config`): فیلد جدید `display_mode` با مقادیر
      مجاز `inline` | `reply` | `both`؛ پیش‌فرض `inline` (سازگار با رفتار قبلی)؛ ورودی نامعتبر/خالی
      به `inline` برمی‌گردد، پس هیچ‌وقت حالت «هیچ‌کدام» ذخیره نمی‌شود.
    - [x] **runtime** (`flow_runtime.php`):
      - `flow_node_display_mode()` — خواندن امن حالت نمایش نود.
      - `flow_runtime_reply_keyboard()` — ساخت کیبورد reply از فرزندان `user` (رد
        `system_menu`/`system_demo`/`root`، `disabled` و دکمه‌های `url` که فقط inline معنی دارند)
        + ردیف ناوبری متنی «🔙 بازگشت»/«🏠 منوی اصلی». اگر چیزی برای نمایش نباشد `null`.
      - `flow_runtime_enter()` بر اساس `display_mode` رندر می‌کند: `reply` فقط کشویی (با fallback به
        inline اگر هیچ دکمهٔ reply نباشد)، `both` پیام+inline و سپس یک پیام «⌨️» با کیبورد کشویی،
        `inline` مثل قبل.
      - `flow_match_child_by_text()` + بلوک تطبیق در `flow_runtime_handle()` — زدن دکمهٔ کشویی
        (متن ساده، بدون callback) را با تطبیق **دقیق** برچسب به فرزند درست برمی‌گرداند و
        `__flowback__`/`__flowhome__` را تشخیص می‌دهد؛ متن نامرتبط هیچ‌وقت ربوده نمی‌شود.
    - [x] **UI** (`panel/js/flow_editor.js` → `NodeForm`): گروه جدید «نحوهٔ نمایش دکمه‌های فرزند»
      پایینِ تیک‌های «بازگشت/منوی اصلی خودکار» با دو چک‌باکس (شیشه‌ای / کشویی). منطق `setDisplayFlag`
      تضمین می‌کند اگر کاربر آخرین تیک را بردارد، تیک دیگر خودکار روشن شود؛ حالت پنهان فقط
      `inline`/`reply`/`both` ذخیره می‌شود.
    - [x] تست‌ها: ۲۵ تست runtime (نرمال‌سازی `display_mode`، ساخت کیبورد reply با رد
      disabled/url/system، تطبیق متن و sentinelها) + ۲۸ تست منطق UI (پایداری «حداقل یکی روشن» در
      همهٔ گذارها) — همگی PASS. `php -l` + `node --check` OK.

  - [x] استپ ۱۲چ — رفع شکستن زنجیرهٔ کشویی + ویرایش‌پذیر کردن دکمه‌های منوی واقعی + پنهان‌کردن پیام بومی  ✅ 2026-06-08
    - بازخورد دقیق کاربر روی ربات live (با تصاویر): زنجیره «سرویس‌های من → تستی۱ → تست۲ → تست۳»
      فقط یک سطح کار می‌کرد؛ با زدن دکمهٔ **کشویی** «تست۲» هیچی نمی‌آمد (نه توضیحات، نه تست۳، نه
      دکمهٔ بازگشت). همچنین دکمه‌های منوی واقعی اصلاً قابل ویرایش نبودند (نمی‌شد شیشه‌ای/کشویی کرد)
      و پیام بومی مزاحم («اشتراک‌های خریداری‌شده…») هم با هر تپ می‌آمد.
    - **ریشه‌یابی باگ زنجیره:** gate در `index.php` فقط وقتی متن ساده را به `flow_runtime_handle`
      می‌فرستاد که نود **منتظر ورودی (await)** باشد. تپِ کشوییِ یک دکمهٔ معمولی await نیست، پس
      اصلاً وارد runtime نمی‌شد و بلوک تطبیق متن هرگز اجرا نمی‌شد.
    - [x] **رفع باگ زنجیره** (`flow_runtime.php` + `index.php`): توابع `flow_state_current_node()` و
      `flow_reply_tap_is_for_flow()` اضافه شد؛ gate در index.php گسترش یافت تا متن ساده‌ای که
      **دقیقاً** با فرزند/ناوبریِ نودِ فعلیِ کاربر مطابقت دارد هم وارد runtime شود (بدون ربودن متن
      نامرتبط). حالا زنجیرهٔ کشویی تا هر عمق ادامه پیدا می‌کند.
    - [x] **ویرایش‌پذیر کردن دکمه‌های منوی واقعی** (`panel/js/flow_editor.js`): دابل‌کلیک روی نود
      `system_menu` دیگر فقط alert نمی‌دهد؛ یک فرم **محدود** باز می‌کند — برچسب و رفتار بومی قفل،
      ولی «نحوهٔ نمایش دکمه‌های فرزند» (شیشه‌ای/کشویی/هر دو) و سرتیتر فرزندان قابل تنظیم. متادیتای
      `menu_key`/`disabled`/`can_toggle`/`system` در round-trip ذخیره حفظ می‌شود.
    - [x] **پنهان‌کردن پیام بومی** (`flow.php` + `flow_runtime.php` + `index.php`): فیلد جدید
      `suppress_native` (فقط برای `system_menu`، پیش‌فرض false). تیک «فقط فرزندان من نمایش داده
      شوند» در فرم. وقتی فعال باشد، `flow_runtime_menu_followup` مقدار `'suppress'` برمی‌گرداند و
      index.php پیش از dispatch بومی `return` می‌کند، پس پیام پیش‌فرض ربات فرستاده نمی‌شود.
    - [x] **followup هماهنگ با display_mode**: `flow_runtime_menu_followup` بازنویسی شد تا فرزندان را
      با همان موتور (`flow_runtime_keyboard`/`flow_runtime_reply_keyboard`) و طبق `display_mode`
      نودِ منو رندر کند و `current_node` کاربر را روی نودِ منو پارک کند تا تپ‌های کشوییِ بعدی
      درست resolve شوند.
    - [x] تست‌ها: ۱۷ تستِ زنجیرهٔ کامل (سرویس‌های من→تستی۱→تست۲→تست۳ در حالت reply، تطبیق هر سطح،
      بازگشت/خانه، رد متن نامرتبط، حالت‌های inline/both) + تست نرمال‌سازی `suppress_native` —
      همگی PASS. `php -l` همهٔ فایل‌ها + `node --check` OK.
...
- [x] استپ ۱۵ — نقشهٔ راه فروشگاه: منوی اصلی (پروفایل/خرید/سبد/پشتیبان) + جدیدترین + دسته‌بندی + گیت پروفایل  ✅ 2026-06-10
  - [x] منوی اصلی فروشگاه `shop_render_main_menu` با ۴ بخش: 👤 پروفایل، 🛍 خرید، 🛒 سبد خرید (با شمارنده)، 🎧 پشتیبان (وصل به `supportbtns` موجود) + سفارش‌های من؛ `/shop` حالا این منو را باز می‌کند
  - [x] زیرمنوی «خرید» `shop_render_buy_menu`: 🆕 جدیدترین محصولات، 🗂 دسته‌بندی، 🔍 جستجوی پیشرفته (وصل به `shopsearch` موجود)، همهٔ محصولات
  - [x] «جدیدترین محصولات»: فقط موجودها، جدیدترین اول، **۵تا ۵تا** با دکمهٔ کشویی «بعدی ⬅️/➡️ قبلی» (`shopnewest_{page}` + `shop_newest_products`) + fallback صفحهٔ خارج از محدوده
  - [x] «دسته‌بندی»: استخراج دسته‌ها از CSV ستون `category` با شمارش (`shop_categories_with_counts`)، hash هشت‌رقمی امن برای callback (نام فارسی در ۶۴ بایت تلگرام جا نمی‌شود)، محصولات هر دسته ۵تایی صفحه‌بندی (`shopcatv_{hash}_{page}`)
  - [x] «پروفایل» `shop_render_profile`: نمایش نام/شماره/آدرس پیش‌فرض + دکمهٔ ثبت شماره (reuse جریان امن `get_number` با request_contact و اعتبارسنجی مالکیت) + دکمهٔ افزودن آدرس (reuse ویزارد آدرس با `shop_addr:0` و پیام پایانی مخصوص پروفایل)
  - [x] **گیت پروفایل**: خرید مستقیم (`shopbuy_`) و تسویهٔ سبد (`cartcheckout`) بدون شماره ثبت‌شده → هدایت به تکمیل پروفایل؛ «افزودن به سبد» برای همه آزاد می‌ماند (دقیقاً طبق نقشهٔ راه) + دکمهٔ گالری به «مشاهدهٔ همهٔ تصاویر و تنوع‌های این محصول» تغییر نام یافت
  - [x] اتصال callbackهای جدید به gate ایزولهٔ فروشگاه در `index.php` (الگوهای `shopmenu/shopbuymenu/shopnewest_/shopcats/shopcatv_/shopprofile/shopsetphone/shopaddaddress`) — رگرسیون VPN صفر + تست منطق ۲۹/۲۹ پاس + `php -l` OK

- [x] استپ ۱۶ — یکپارچه‌سازی فروشگاه با منوی اصلی، کیف پول و مستندات  ✅ 2026-06-10
  - [x] در حالت فروشگاه (`panel_mode() !== 'vpn'`)، دکمهٔ «خرید» منوی اصلی ربات (text_sell / callback `buy` / `/buy`) حالا **منوی اصلی فروشگاه** را باز می‌کند نه ویزارد VPN — گارد کامل: در حالت VPN هیچ تغییری نیست (handler قبلی دست‌نخورده پایین‌تر اجرا می‌شود)؛ پشتیبانی از لیبل سفارشی دکمه با `bot_label('sell')`
  - [x] UX کسری موجودی: به‌جای بن‌بست، پیام کسری + دکمهٔ «💳 افزایش موجودی» (اتصال مستقیم به جریان `Add_Balance` موجود = همهٔ درگاه‌ها/کارت‌به‌کارت) + بازگشت به سبد/محصول — در هر دو مسیر خرید مستقیم و تسویهٔ سبد؛ سبد محفوظ می‌ماند
  - [x] «سفارش‌های من» حالا دکمهٔ بازگشت «🛍 فروشگاه» دارد (قبلاً بدون کیبورد رها می‌شد)
  - [x] به‌روزرسانی `docs/store-guide.md`: نمودار جریان خرید با منوی ۴بخشی جدید، گیت پروفایل، صفحه‌بندی ۵تایی و مسیر شارژ کیف پول
  - [x] `php -l` هر دو فایل OK + تست‌های استپ ۱۵ همچنان ۲۹/۲۹ پاس
...
- [استپ E] کلیدهای زبان `serviceCol*` / `serviceDetail*` در `panel/service.php` نیز مثل کلیدهای محصول
  درهم‌ریخته‌اند (مثل `serviceColAmount => 'یوزرنیم'`). چون در service.php به‌صورت سازگار مصرف
  می‌شوند خروجی درست رندر می‌شود؛ پس فقط مسئلهٔ خوانایی کد است. در فاز بعدی با همان روش امن
  rename (دو-مرحله‌ای) اصلاح می‌شود. ریسک/بازده فعلی پایین → عمداً به این فاز موکول نشد.

---

## 🔍 ممیزی جامع کیفیت/منطق (Audit) — 2026-06-09
> درخواست صاحب پروژه: «مو به مو بررسی، یافتن همهٔ باگ‌ها و مشکلات منطقی/UX».
> طبق AGENT_RULES باگ‌ها فقط **ثبت** شدند (اصلاح نشد). هر مورد → یک استپ رفع پیشنهادی.

### A) آپلود تصویر محصول — UX شکسته (شکایت مستقیم کاربر)
- [Audit-1] فرم «افزودن محصول» (`panel/product.php`) **هیچ فیلد آپلود تصویری ندارد**. آپلود
  فقط بعد از ساختِ محصول، از طریق یک دکمه با **آیکون «چشم» (eye)** و tooltip «رسانهٔ محصول»
  در ردیف جدول ممکن است → کاربر اصلاً پیدایش نمی‌کند. باید: (۱) آیکون به «تصویر/گالری» عوض شود،
  (۲) یک کشوی آپلود مستقیم داخل مودال افزودن/ویرایش محصول اضافه شود (یا حداقل دکمهٔ واضح
  «🖼 تصاویر محصول»). فایل‌ها: `panel/product.php` خط ۱۵۱-۱۵۳، `panel/product_media.php`.
- [Audit-2] رسانه به تلگرام با **URL محلی** (`$domainhosts/uploads/...`) ارسال می‌شود
  (`function.php` ~۲۹۷۷-۲۹۹۲)؛ `telegram_file_id` هنگام آپلود پنل **هرگز پر نمی‌شود**. اگر
  دامنه عمومی/HTTPS معتبر نباشد تلگرام نمی‌تواند فایل را بگیرد و رسانه **بی‌صدا** ارسال نمی‌شود.
  راه‌حل: بعد از آپلود، یک‌بار به تلگرام بفرست و `file_id` برگشتی را ذخیره کن (سریع‌تر و پایدارتر).

### B) حمل‌ونقل — قیمت دستی به‌جای API (شکایت مستقیم کاربر)
- [Audit-3] `calc_shipping_cost()` (`function.php` ~۴۲۷۵) فقط یک **عدد ثابت** برمی‌گرداند
  (`carrier.cost` یا `default_cost`). **هیچ فراخوانی API** به پست/تیپاکس/چاپار/ماهکس انجام
  نمی‌شود و **وزن محصول در محاسبه نقشی ندارد**. API credentials در `panel/shipping.php` ذخیره
  می‌شوند ولی **استفاده نمی‌شوند** (dead config). راه‌حل: تابع `carrier_quote($carrier,$weight,
  $dest_city,$creds)` که cURL به API هر شرکت بزند؛ fallback به نرخ دستی فقط وقتی API نبود.
- [Audit-4] فیلد «وزن (گرم)» در محصول با hint «برای محاسبهٔ هزینهٔ ارسال» وجود دارد
  (`function.php` ~۱۸۶۶) اما **هیچ‌جا مصرف نمی‌شود** → وعدهٔ توخالی. باید به Audit-3 وصل شود.
- [Audit-5] **تعریف دوگانهٔ هزینهٔ ارسال** (تناقض منطقی): یک‌بار در attribute محصول
  `shipping_methods` (repeater نام/هزینه/روز) و یک‌بار در `panel/shipping.php` (carrier cost).
  `calc_shipping_cost` فقط دومی را می‌خواند؛ `shipping_methods` محصول عملاً بی‌اثر است. باید
  یکی حذف یا دو لایه به‌روشنی ادغام شوند (سطح فروشگاه vs سطح محصول).

### C) دو سیستم n8n موازی و گیج‌کننده (شکایت مستقیم کاربر)
- [Audit-6] **دو مسیر مجزا برای اتصال n8n** وجود دارد که همپوشانی دارند و کاربر نمی‌داند کدام
  برای چیست:
  1. صفحهٔ «اتوماسیون» (`panel/automation.php`): Outbound Webhooks + Inbound API token + کلید
     HMAC (سیستم استپ ۱۱).
  2. نود `n8n` داخل «ویرایشگر درختی» (`flow.php` خط ۸۰۶ به بعد): با `n8n_mode` (async/sync)،
     `n8n_endpoint`، `n8n_timeout_sec` (سیستم استپ ۱۲).
  راه‌حل: یک «منبع حقیقت» واحد برای endpoint/secret n8n؛ نود درختی باید از همان تنظیمات صفحهٔ
  اتوماسیون استفاده کند (یا حداقل با یک متن راهنما تفاوت دو حالت توضیح داده شود).
- [Audit-7] آپشن‌های نود n8n (async/sync/timeout/tag/endpoint) **بدون توضیح کاربردی** هستند؛
  برای کاربر غیرفنی روشن نیست async چه فرقی با sync دارد یا «callback» یعنی چه. نیاز به
  hint/راهنمای فارسی داخل فرم نود.

### D) برندینگ/اصطلاحات جدا از درختی (شکایت مستقیم کاربر)
- [Audit-8] صفحهٔ «برندینگ و اصطلاحات» (`panel/labels.php`) لیبل **همان دکمه‌هایی** را ویرایش
  می‌کند که در «ویرایشگر درختی» (`flow.php`) ساخته/چیده می‌شوند → دو جای جدا برای یک کار.
  منطقی‌تر: ویرایش نام دکمه **مستقیماً داخل نودِ درختی** (یا حداقل دکمهٔ «ویرایش متن این دکمه»
  که به همان فیلد لینک بدهد). صفحهٔ برندینگ فقط برای «واژگان فروشگاه» (مشتری/محصول/سفارش) بماند.

### E) باگ سیستماتیک نام‌گذاری کلیدهای زبان (بدهی فنی شدید)
- [Audit-9] در `lang/fa.php` و `lang/en.php` **نام صدها کلید با مقدارشان بی‌ربط است** (نگاشت
  خودکارِ غلط). نمونه‌ها: `productColActions => 'قیمت'`، `productColName => 'هنوز محصولی
  ثبت نکرده‌اید'`، `layoutFooterCopyright => 'داشبورد'`، `serviceColAmount => 'یوزرنیم'`،
  `userColName => 'ت'`. چون **کد هم همان کلید بی‌ربط را در جای درست صدا می‌زند، خروجی نهایی
  در مرورگر معمولاً درست است** (باگ نمایشی نیست) — ولی برای هرکس که بخواهد متن را پیدا/ویرایش
  کند **کابوس نگهداری** است و دقیقاً حس «گیج‌کننده» را می‌سازد. راه‌حل: یک‌بار اسکریپت
  بازنویسی کلیدها بر اساس محتوا (rename بزرگ، هماهنگ در کد + ۴ فایل زبان). ریسک بالا → استپ مجزا.

### F) به‌هم‌ریختگی بصری ستون‌های جدول محصول
- [Audit-10] در `panel/product.php` (خط ۱۱۸-۱۲۹) ترتیب/برچسب ستون‌های `<thead>` با ترتیب
  دادهٔ `<td>`ها هم‌خوان نیست (مثلاً سرستون اول `#` ولی برچسب ستون نام محصول از کلید
  `productColPrice` می‌آید). باید سرستون‌ها با داده‌ها دقیق منطبق و با کلید درست شوند.
  (وابسته به Audit-9.)

### استپ‌های رفع پیشنهادی (به‌ترتیب اولویت، هرکدام بعد از تأیید)
- [x] استپ A — رفع UX آپلود تصویر محصول (Audit-1,2)  ✅ 2026-06-09
  - آیکون «چشم» → آیکون «تصویر» + بَج تعداد رسانه روی دکمه (`panel/product.php`, `icons.php`)
  - راهنمای واضح آپلود در مودال افزودن + دکمهٔ مستقیم «مدیریت رسانه» در مودال ویرایش (`product.php`, `js/product.js`)
  - رفع Audit-2: تابع `product_media_cache_telegram_id()` بعد از آپلود، فایل را یک‌بار به ادمین می‌فرستد و `file_id` دائمی را ذخیره می‌کند تا ارسال در ربات بدون URL عمومی هم پایدار باشد (`function.php`, `product_media.php`)
  - تابع کمکی `product_media_count()` افزوده شد؛ `product_media_add()` حالا id رکورد را برمی‌گرداند
- [x] استپ B — محاسبهٔ هزینهٔ ارسال بر پایهٔ API + وزن، و حذف تناقض دوگانه (Audit-3,4,5)  ✅ 2026-06-09
  - `calc_shipping_cost()` بازنویسی شد با ترتیب: رایگان → API زنده → نرخ پلکانی وزنی → ثابت شرکت → پیش‌فرض (`function.php`)
  - تابع `carrier_quote()` (adapter API نرخ‌دهی با `quote_url`) + `cart_total_weight()` (جمع وزن اقلام) اضافه شد
  - رفع Audit-4: فیلد وزن محصول حالا واقعاً در محاسبه استفاده می‌شود (نرخ پلکانی base + per_kg)
  - **هزینهٔ ارسال به جریان checkout سبد وصل شد** (قبلاً اصلاً اضافه نمی‌شد!) + ذخیره در `Payment_report.shipping_cost` + نمایش در پیام پرداخت
  - رفع Audit-5: توضیح روشن ترتیب اولویت در پنل + اعلام اینکه `shipping_methods` محصول فقط نمایشی است
  - پنل `shipping.php`: کادر نرخ وزنی سراسری + فیلد وزنی per-carrier + فیلد `quote_url` برای تیپاکس/پست
  - ۷ تست واحد منطق محاسبه: همه PASS
- [x] استپ C — یکپارچه‌سازی اتصال n8n + راهنمای فارسی آپشن‌ها (Audit-6,7)  ✅ 2026-06-09
  - رفع Audit-6 (دو سیستم گیج‌کننده): کارت توضیح شفاف در هر دو سو افزوده شد —
    صفحهٔ «اتوماسیون و n8n» = رویدادهای کلی خودکار ربات؛ نود n8n درختی = اجرا فقط هنگام کلیک کاربر روی همان دکمه (`panel/automation.php`, `panel/js/flow_editor.js`)
  - **تک‌منبع‌حقیقت (single source of truth):** نود n8n می‌تواند یک «اتصال ذخیره‌شده» (endpoint نام‌دار سراسری) را انتخاب کند → آدرس و کلید امضا خودکار از `automation_config` گرفته می‌شود؛ دیگر لازم نیست URL دوبار وارد شود
  - بک‌اند: `flow_n8n_resolve_ref()` + `flow_n8n_endpoint_options()` افزوده شد؛ `flow_n8n_send()` حالا ref را رزولو کرده و در صورت استفاده از endpoint سراسری، body را با همان HMAC secret امضا می‌کند (هدر `X-Signature`) (`flow.php`)
  - فیلد `n8n_endpoint_ref` به sanitize نود + کانفیگ پیش‌فرض ویرایشگر اضافه شد؛ dropdown «اتصال ذخیره‌شده» در پنل نود (`panel/flow.php`, `flow_editor.js`)
  - رفع Audit-7: راهنمای فارسی آپشن‌ها (async/sync/timeout/پیام انتظار/داده‌های ارسالی) از قبل موجود بود؛ توضیح تفاوت دو سیستم تقویت شد
  - ۶ تست واحد منطق رزولو/امضا: همه PASS؛ lint همهٔ فایل‌ها OK
- [x] استپ D — انتقال ویرایش نام دکمه به داخل نود درختی (Audit-8)  ✅ 2026-06-09
  - رفع Audit-8 (هم‌پوشانی برندینگ ↔ درخت): ویرایش نام دکمه‌ها از `panel/labels.php` حذف شد و به نود درختی منتقل شد
  - حالا برچسب نودهای `system_menu` در ویرایشگر قابل ویرایش است؛ پس از ذخیره، `flow_sync_menu_labels()` نام را در همان store قبلی (`bot_label`) ذخیره می‌کند → در کیبورد ربات اعمال می‌شود
  - کارت «لیبل دکمه‌های ربات» در `labels.php` با یک راهنمای واضح (لینک به ویرایشگر درختی) جایگزین شد؛ این صفحه فقط برای «واژگان فروشگاه» و «برندینگ» باقی ماند
  - تأیید دومرحلهٔ ترسناک برای rename نود `system_menu` حذف شد (تغییر نام امن است؛ callback قفل است)؛ متن راهنمای فیلد به‌روزرسانی شد
  - بک‌اند: `flow_sync_menu_labels()` در `set_button_flow()` (مرحلهٔ ۵) فراخوانی می‌شود؛ خالی/پیش‌فرض = حذف override
  - ۵ تست واحد منطق sync لیبل: همه PASS؛ lint همه OK
- [x] استپ E — بازنویسی نام کلیدهای زبان بر اساس محتوا + رفع ستون‌ها (Audit-9,10)  ✅ 2026-06-09
  - تشخیص دقیق: کلیدهای محصول/سرویس در هر ۴ زبان (fa/en/ru/zh) به‌شکل **یکسان و سازگار** درهم‌ریخته بودند (مثل `productColPrice => 'نام محصول'`، `productColActions => 'قیمت'`) — یعنی خروجی *درست* رندر می‌شد ولی نام کلیدها گمراه‌کننده بود (تله برای توسعه‌دهندهٔ بعدی)
  - تأیید شد این کلیدها فقط در `panel/product.php` + ۴ فایل زبان مصرف می‌شوند (هیچ فایل دیگری) → rename ایمن
  - ۲۸ کلید با اسکریپت دو-مرحله‌ای (temp-token برای جلوگیری از تصادم) به نام‌های معنادار تغییر یافت — هم‌زمان در `product.php` و هر ۴ زبان (مثلاً `productColName='نام محصول'`, `productColPrice='قیمت'`, `productColVolume='حجم'`, `productColDuration='مدت'`, `productColPanel='پنل'`, `productColCategory='دسته'`, `productColCode='کد'`, `productColActions='عملیات'`)
  - رفع Audit-10: سرستون‌های جدول حالا به‌ترتیب درست و معنادار = # | نام | قیمت | حجم | مدت | پنل | دسته | کد | عملیات و دقیقاً با سلول‌های داده هم‌تراز است
  - راستی‌آزمایی: lint هر ۵ فایل OK؛ هیچ توکن temp باقی نمانده؛ هر ۶۴ کلید مصرف‌شده در product.php در هر ۴ زبان resolve می‌شود
  - یادداشت: کلیدهای سرویس (`serviceCol*`) هم درهم‌ریخته‌اند اما چون در `service.php` به‌صورت سازگار مصرف می‌شوند و رندر درست است، در فهرست باگ‌ها برای فاز بعدی ثبت شد (ریسک/بازده پایین)
...
## بهبود UX فرم افزودن محصول (درخواست کاربر) — 2026-06-09
- [x] «نوع محصول» به **ابتدای** فرم افزودن منتقل شد (قدم ۱) با راهنمای واضح
- [x] فیلدهای مخصوص VPN (حجم/مدت/پنل) با `data-ptype-only="vpn"` علامت‌گذاری شدند؛ تابع
  `applyTypeVisibility()` در `product.js` آن‌ها را با تغییر نوع، نمایش/مخفی و `disabled` می‌کند
  → برای «کالای فیزیکی» و … دیگر فیلد/مثال نامرتبط VPN دیده نمی‌شود (۶ تست منطق allow-list PASS)
- [x] آپلود تصویر **داخل همان فرم افزودن** اضافه شد (drag&drop + نمایش نام فایل) — دیگر نیازی به
  ویرایش مجدد محصول برای افزودن عکس نیست؛ form به `multipart/form-data` تبدیل شد
- [x] بک‌اند: منطق آپلود به تابع مشترک `product_media_handle_upload()` در `function.php` منتقل شد
  (DRY)؛ هم `product_media.php` و هم مسیر افزودن محصول از آن استفاده می‌کنند؛ فایل‌های `media[]`
  هنگام ثبت محصول جدید پردازش و در `product_media` ثبت + file_id تلگرام کش می‌شوند
- [x] پیام موفقیت تعداد رسانهٔ آپلودشده را گزارش می‌دهد؛ دکمهٔ برچسب‌دار «افزودن تصویر/تصاویر(n)»
  در جدول برای افزودن بعدی باقی است
- lint هر ۴ فایل (function.php, product.php, product_media.php, product.js) OK

## ۵ نکتهٔ فرم محصول (درخواست کاربر) — 2026-06-09
- [x] **نکتهٔ ۱ (ویرایش‌پذیری):** تأیید شد محصول پس از ایجاد قابل ویرایش است؛ فرم ویرایش
  بازنویسی شد تا type-first و تمیزتر باشد (فیلدهای VPN فقط برای نوع VPN دیده می‌شوند).
- [x] **باگ ۲ (بسته‌شدن فرم هنگام انتخاب متن):** در `app.js` بستن مودال با کلیک روی پس‌زمینه
  حالا به mousedown+mouseup روی همان پس‌زمینه نیاز دارد → drag-select برای کپی متن دیگر فرم را نمی‌بندد.
- [x] **باگ ۳ (placeholder نامرتبط):** آرایهٔ `examples` به هر نوع محصول در `product_types()` اضافه شد
  + اتریبیوت `data-example` روی فیلدها + `applyTypeExamples()` در JS → با انتخاب «کالای فیزیکی»
  دیگر مثال «۵۰ گیگ یک ماهه» نمایش داده نمی‌شود؛ مثال‌ها با نوع انتخاب‌شده هماهنگ‌اند.
- [x] **نکتهٔ ۴ (دسته‌بندی‌ها):** صفحهٔ مجزای `categories.php` (افزودن/ویرایش/حذف + شمارش مصرف) ساخته شد؛
  فیلد دسته در فرم محصول به انتخاب چندتایی (چیپ/چک‌باکس) از فهرست موجود تبدیل شد + افزودن سریع دستهٔ جدید
  + لینک به صفحهٔ مدیریت. ذخیره به‌صورت CSV سازگار با قبل در ستون موجود `product.category`؛ rename/delete دسته
  در همهٔ محصولات هم‌گام می‌شود. لینک منو «دسته‌بندی‌ها» اضافه شد.
- [x] **نکتهٔ ۵ (پس‌کد/واریانت):** ستون اختیاری «پس‌کد» (مثلاً ۲۳۴۵-۱، ۲۳۴۵-۲) به repeater «تنوع محصول»
  در نوع «کالای فیزیکی» اضافه شد؛ کنار رنگ/سایز/SKU/موجودی/اختلاف قیمت. اختیاری است و فقط در صورت نیاز پر می‌شود.
- lint همهٔ فایل‌ها OK؛ تست‌های ایزولهٔ product_types (۵ نوع + ستون variant_code) PASS.
