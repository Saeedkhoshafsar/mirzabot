...
8. **انبار و موجودی (Inventory):**
   موجودی در سطح محصول (`stock`) و در سطح واریانت (هر رنگ/سایز). کاهش خودکار موجودی هنگام فروش، نمایش «ناموجود»، هشدار کم‌بودن موجودی در پنل.

9. **جستجو و فیلتر پیشرفته (Search & Filter):**
   در پنل: جستجوی محصول/سفارش/کاربر بر اساس نام، SKU، کد رهگیری، نوع محصول، دسته، بازهٔ قیمت، وضعیت، تاریخ. در ربات: جستجوی محصول و فیلتر بر اساس دسته/قیمت.
...
- [ ] دسته‌بندی محصول (موجود: `api/category.php`) — اتصال به فیلتر/جستجو در استپ ۸د
- [ ] موجودی/انبار و کاهش خودکار + هشدار — استپ ۸ج
...
- [ ] وضعیت سفارش (در انتظار→پرداخت→ارسال→تحویل→لغو) — استپ ۸ب
- [ ] کد رهگیری پستی + شرکت ارسال — استپ ۸ب
- [ ] روش‌های ارسال چندگانه (پست/تیپاکس/پیک) + هزینه — استپ ۵ب (تعریف)، ۷/۸ب (اعمال)
- [ ] «سفارش‌های من» در ربات + اطلاع‌رسانی تغییر وضعیت — استپ ۸ب

### مشتری
- [x] کیف‌پول/موجودی کاربر (موجود)
- [ ] آدرس‌های مشتری (چند آدرس، پیش‌فرض) — استپ ۸ج
- [ ] تاریخچهٔ خرید کاربر — استپ ۸ب (سفارش‌های من)

### مدیریت و گزارش (پنل)
- [x] داشبورد/آمار (موجود: `panel/` + `api/statbot.php`)
- [ ] جستجو و فیلتر پیشرفتهٔ محصول/سفارش/کاربر — استپ ۸د
...
- [ ] قالب‌های آماده + ویزارد — استپ ۹
- [ ] دکمه‌های سفارشی + n8n — استپ ۱۱
- [ ] مستندات و نمونهٔ فروشگاه — استپ ۱۰
...
- [ ] استپ ۸ب — سفارش، وضعیت و کد رهگیری (Orders & Tracking)
  - [ ] گسترش `Payment_report` (non-destructive): `order_status` (pending|paid|preparing|shipped|delivered|canceled|refunded)، `tracking_code`، `shipping_carrier` (پست/تیپاکس/...)، `shipping_method`، `shipping_cost`، `address_id`، `product_id`، `variant` (JSON رنگ/سایز انتخابی)، `quantity`
  - [ ] صفحهٔ پنل `panel/orders.php` (self-contained، per-bot): لیست سفارش‌ها با فیلتر وضعیت، جزئیات سفارش، تغییر وضعیت، ثبت کد رهگیری + انتخاب شرکت ارسال
  - [ ] اطلاع‌رسانی خودکار به کاربر در ربات هنگام تغییر وضعیت (ارسال شد + کد رهگیری / تحویل شد)
  - [ ] دکمهٔ «سفارش‌های من» در ربات: کاربر وضعیت و کد رهگیری سفارش‌هایش را ببیند
  - [ ] تابع `set_order_status()` + `notify_order_update()` در `function.php`
  - [ ] رگرسیون: سفارش‌های VPN فعلی (که وضعیت ندارند) دست‌نخورده

- [ ] استپ ۸ج — آدرس مشتری و انبار/موجودی (Address & Inventory)
  - [ ] جدول `customer_address` (id, user_id, bot_id, full_name, phone, province, city, postal_code, address, is_default, created_at)
  - [ ] دریافت/انتخاب آدرس در ربات هنگام خرید محصول فیزیکی (افزودن آدرس جدید / انتخاب آدرس قبلی)
  - [ ] صفحهٔ پنل: نمایش آدرس مشتری در جزئیات سفارش
  - [ ] **منطق موجودی**: کاهش خودکار `stock` (محصول یا واریانت انتخابی) هنگام فروش موفق
  - [ ] نمایش «ناموجود» و جلوگیری از خرید وقتی موجودی صفر است (محصول/واریانت)
  - [ ] هشدار کم‌بودن موجودی در پنل (آستانهٔ قابل تنظیم) + بَج «ناموجود/کم» در لیست محصولات
  - [ ] تابع `decrement_stock($product_id, $variant, $qty)` + `check_availability()` در `function.php`
